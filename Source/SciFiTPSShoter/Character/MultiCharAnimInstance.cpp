...
void UMultiCharAnimInstance::NativeUpdateAnimation(float DeltaTime)
{
	...
	bEliminated = MultiCharacter->IsElimmed();
	...
	if (bWeaponEquipped && EquippedWeapon && EquippedWeapon->GetWeaponMesh() && MultiCharacter->GetMesh())
	{
		...
		{
			bLocallyControlled = true;  // EquippedWeapon->GetWeaponMesh()->GetSocketTransform(FName("hand_r"), ERelativeTransformSpace::RTS_World);
